...
describe('FundMe', async () => {
  ...
  describe("withdraw", async () => {
    ...
    it("withdraw by deployer", async () => {
      ...
      const startingFundMeBalance = await provider?.getBalance(fundMeAddress) || BigInt(0);
      const startingDeployerBalance = await provider?.getBalance(deployer) || BigInt(0);

      const transactionResponse = await fundMe.withdraw();
      const transactionReceipt = await transactionResponse.wait() || {gasUsed: BigInt(0), gasPrice: BigInt(0)};
